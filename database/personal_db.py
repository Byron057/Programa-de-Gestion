...
def guardar_personal(cedula, nombres, apellidos, telefono, correo, provincia, ciudad, direccion,foto):
    error= True
    try:
...
def editar_datos_personal(cedula, nombres, apellidos, telefono, correo, provincia, ciudad, direccion,foto,id):
    error= True
    try:
        conn= db_core.conectBaseDeDatos()
        query= conn.cursor()
